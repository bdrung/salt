...
def test_gnu_slash_linux_in_os_name():
    ...
    with patch.object(
        ...
        core, "freedesktop_os_release", missing_os_release_mock
    ), patch.object(
        core, "_parse_lsb_release", empty_mock
    ), patch.object(
...
def test_suse_os_from_cpe_data():
    ...
    with patch.object(
        ...
        core, "freedesktop_os_release", os_release_mock
    ), patch.object(
        core, "_parse_lsb_release", empty_mock
    ), patch.object(
        core, "_linux_distribution", distro_mock
...
def _run_os_grains_tests(
    os_release_data, lsb_distro_information, os_release_map, expectation
):
    ...
    # - Skip the init grain compilation (not pertinent)
    # - Skip all the /etc/*-release stuff (not pertinent)
    # - Mock lsb_release (import will fail if not lsb_distro_information)
    # - Mock _linux_distribution to give us the OS name that we want
    # - Mock the osarch
    ...
    with patch.object(
        ...
        core, "freedesktop_os_release", freedesktop_os_release_mock
    ), patch.object(
        core, "_parse_lsb_release", empty_mock
    ), patch(
...
def _run_suse_os_grains_tests(os_release_data, os_release_map, expectation):
    ...
    _run_os_grains_tests(os_release_data, None, os_release_map, expectation)
